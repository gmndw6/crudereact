...
function Home() {
  ...
  const handleDelete = (id) => {
    const isDelete = employee.findIndex((e) => e.id === id);
                  ...
                  <StyledTableCell>
                    <Link to="/view">
                      <Tooltip title="View" TransitionComponent={Fade} TransitionProps={{ timeout: 400 }} placement="top-start">
                        <RemoveRedEyeIcon
                          onClick={() => handleView(item.id, item.name, item.email, item.contactNumber)}
                          style={{ color: '#288BA8' }}
                        />
                      </Tooltip>
                    </Link>
                    <Link to="/edit">
                      <Tooltip title="Edit" TransitionComponent={Fade} TransitionProps={{ timeout: 400 }} placement="top-start">
                        <ModeEditOutlineIcon
                          style={{ color: '#27A567' }}
                          onClick={() => handleEdit(item.id, item.name, item.email, item.contactNumber)}
                        />
                      </Tooltip>
                    </Link>
